    ...
    def forward(self, xyz1, xyz2, points1, points2, flow, flow_gt, certainty, uncertainty = 0.5):
        ...
        if self.training:
            ...
            for i in range(self.iters):
                ...
                #certainty_new = certainty_new.permute(0,2,1)
...
class PointConvBidirection(nn.Module):
    ...
    def forward(self, xyz1, xyz2, color1, color2, gt_flow, mask, uncertainty = 0.5):
        ...
        # add
        # gt_flow
        l3_label = index_points_gather(gt_flow, idx1s[2])
        l2_label = index_points_gather(gt_flow, idx1s[1])
        l1_label = index_points_gather(gt_flow, idx1s[0])
        l0_label = gt_flow
         
        l0_mask = torch.split(mask,16384,1)[0]
        l1_mask = index_points_gather(l0_mask,idx1s[0])
        l2_mask = index_points_gather(l1_mask,idx1s[1])
        l3_mask = index_points_gather(l2_mask,idx1s[2])
        ...
        labels = [l0_label,l1_label,l2_label,l3_label]
        mask_list = [l0_mask,l1_mask,l2_mask,l3_mask]
        if self.training:
            return flows, fps_pc1_idxs, fps_pc2_idxs, pc1, pc2, loss_l2, loss_l1, loss_l0, mask_list, labels
        else:
            return flows, fps_pc1_idxs, fps_pc2_idxs, pc1, pc2, mask_list, labels

def multiScaleLoss(pred_flows, gt_flow, fps_idxs, mask, loss_f2 = None,loss_f1 = None,loss_f0 = None, alpha = [0.02, 0.04, 0.08, 0.16]):
    ...
    for i in range(num_scale):
        ...
        total_loss += alphas[i] * (torch.norm(diff_flow, dim = 2,keepdim=True)*mask[i]).sum(dim = 1).mean()
