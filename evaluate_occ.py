...
# from models_bid_lighttoken_res0_iter_bifeat_feat import PointConvBidirection
# from models_bid_lighttoken_res0_iter_bifeat_feat import multiScaleLoss
...
def main():
    ...
    os.system('cp %s %s' % ('model_difflow_occ.py', log_dir))
    os.system('cp %s %s' % ('pointconv_util.py', log_dir))
    os.system('cp %s %s' % ('evaluate_occ.py', log_dir))
    os.system('cp %s %s' % ('config_evaluate_occ.yaml', log_dir))
    ...
    for i, data in tqdm(enumerate(val_loader, 0), total=len(val_loader), smoothing=0.9):
        ...
        #mask = mask.unsqueeze(2).cuda().float().float()
        ...
        with torch.no_grad(): 
            ...
            starter.record()
            pred_flows, fps_pc1_idxs, _, _, _, _mask, gt_flows = model(pos1, pos2, norm1, norm2, flow, mask, uncertainty)
            ender.record()
            torch.cuda.synchronize()
            curr_time = starter.elapsed_time(ender)/1000
            total_time += curr_time

            loss = multiScaleLoss(pred_flows, flow, fps_pc1_idxs, _mask)

            full_flow = pred_flows[0][0].permute(0, 2, 1)
    ...
    mean_time = total_time / total_seen
