...
def main():
    ...
    #os.system('cp %s %s' % ('model_difflow.yaml', log_dir))
    ...
    for epoch in range(init_epoch, args.epochs):
        ...
        for i, data in tqdm(enumerate(train_loader, 0), total=len(train_loader), smoothing=0.9, ncols=100,mininterval=60.0):
            ...
            model = model.train() 
            pred_flows, fps_pc1_idxs, _, _, _ ,loss_f2,loss_f1,loss_f0= model(pos1, pos2, norm1, norm2, flow, uncertainty = uncertainty)
            # sf = sf.detach().cpu().numpy()
            # exit()

            loss = multiScaleLoss(pred_flows, flow, fps_pc1_idxs,loss_f2,loss_f1,loss_f0)
...
def eval_sceneflow(model, loader, multiScaleLoss):
    ...
    for batch_id, data in tqdm(enumerate(loader), total=len(loader), smoothing=0.9, mininterval=10.0):
        ...
        with torch.no_grad():
            pred_flows, fps_pc1_idxs, _, _, _ = model(pos1, pos2, norm1, norm2, flow)

            eval_loss = multiScaleLoss(pred_flows, flow, fps_pc1_idxs)

            epe3d = torch.norm(pred_flows[0][0].permute(0, 2, 1) - flow, dim = 2).mean()
